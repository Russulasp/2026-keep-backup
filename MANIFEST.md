...
## 3. 前提・制約（Constraints）
- 実行環境は Windows + WSL(Ubuntu) + Docker（**実行はコンテナを正とする**）
...
- 出力は WSL 側のプロジェクトディレクトリ配下へ保存する（Docker bind mount 経由）
...
- 任意：HTML（閲覧用）は後付け可
- 取得と解釈の責務分離
  - backup 実行時は Keep DOM スナップショット（HTML）を `logs/artifacts/` に保存する
  - DOM 解釈は別モードで実行できるようにし、保存済み DOM から JSON を再生成可能にする
- 出力先（固定）
  - `backups/YYYY-MM-DD/keep.json`
  - `logs/run_YYYY-MM-DD_HHMMSS.log`
  - `logs/artifacts/dom_snapshot_YYYY-MM-DD_HHMMSS.html`
- 保持（暫定）
...
## 6. 成功条件（Done）
### Phase 1（ローカル保管MVP）
- `docker compose`（およびその薄いラッパ `make`）の入口で開始できる
...
### Phase 2（将来）
- Windows 入口バッチ（`backup_keep.bat`）を追加し、WSL/Docker 実行へ委譲できる
...
## 12. 実行入口の原則（Container-first）
- 通常運用の実行主体は Docker に統一する（WSL は Docker 呼び出しの中継）
- 入口コマンドは「Docker 実行に収束する」範囲で追加を許容する
  - 追加時は既存入口の重複実装を避け、同じ Docker コマンドへ委譲する
  - ローカルとCIで再利用できる命名・挙動を優先する
- `uv run ...` の直実行は開発時の一時検証に限定し、標準運用の入口にはしない
